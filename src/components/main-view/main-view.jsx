...
export const MainView = () => {
	...
	const getUniqueSimilarMovies = (movies) => {
		return Array.from(new Set(movies.map((movie) => movie.id))).map((id) =>
	...
	const filterMovies = (movies) => {
		return movies.filter((movie) => {
			return (
				...
				(directorFilter
					? movie.director.name.toLowerCase().includes(directorFilter.toLowerCase())
					: true) &&
				(actorFilter
					? movie.actors &&
		...
		const fetchData = async () => {
			try {
				...
				const favouritesResponse = await fetch(
					`https://dojo-db-e5c2cf5a1b56.herokuapp.com/users/${user.username}`,
					{
						headers: {
							Authorization: `Bearer ${storedToken}`,
							'Content-Type': 'application/json',
				...
				const releaseYears = [...new Set(moviesFromApi.map((movie) => movie.releaseYear))].sort(
					(a, b) => a - b
				);
				const directors = [...new Set(moviesFromApi.map((movie) => movie.director.name))].sort();
				const actors = [
					...new Set(moviesFromApi.flatMap((movie) => movie.actors.map((actor) => actor.name))),
				].sort();
		...
		try {
			...
			setAllMovies((prevMovies) =>
				prevMovies.map((movie) =>
					movie.id === movieID ? { ...movie, isFavourite: !isFavourite } : movie
				)
			);
		} catch (error) {
								...
								<Row>
									...
									<Row className='g-3 d-flex justify-content-between'>
										<Col
											...
											md={3}
											lg={2}
											className='d-flex justify-content-md-start mb-3'>
											{/* Genre Filter */}
											<DropdownButton
												id='genre-filter'
										...
										<Col
											...
											md={3}
											lg={2}
											className='d-flex justify-content-sm-start justify-content-md-center mb-3'>
											{/* Release Year Filter */}
											<DropdownButton
										...
										<Col
											...
											md={3}
											lg={2}
											className='d-flex justify-content-sm-start justify-content-md-end mb-3'>
											{/* Actor Filter */}
											<DropdownButton
												id='actor-filter'
														...
														<ol className='breadcrumb'>
															...
															{releaseYearFilter && (
																<li className='breadcrumb-item'>
																	...
																	<Link to={`/movies/release-year/${releaseYearFilter}`}>
																		{releaseYearFilter}
																	</Link>
																	<button
																		className='btn btn-link'
																		onClick={() => setReleaseYearFilter('')}>
																		<i className='bi bi-x'></i>
																	</button>
																</li>
															)}
									...
									{filterMovies(allMovies).length === 0 ? (
										<Row className='g-3 mb-5 d-flex justify-content-center'>
											<Col>
												<h3>No movies for the applied filters found!</h3>
											</Col>
										</Row>
									) : (
										// filtered movies
													...
													<Col
														...
														<MovieCard
															...
															isFavourite={
																...
																favourites.some(
																	(fav) => fav.movieId.toString() === movie.id.toString()
																)
															}
															onToggleFavourite={(movieId, isFavourite) =>
																toggleFavourite(movieId, isFavourite)
															}
														/>
													</Col>
												))
