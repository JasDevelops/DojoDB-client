...
export const MovieView = ({ allMovies, favourites = [], onToggleFavourite }) => {
	...
	const isFavourite =
		Array.isArray(favourites) && favourites.some((fav) => fav.movieId === movie.id);

	// Similar movies
	const similarMovies = allMovies.filter((simMovie) => simMovie.id !== movie.id).slice(0, 3);
								...
								{movie.actors && movie.actors.length > 0 ? (
									movie.actors.map((actor, index) => (
										<p key={index}>
											...
											as "{actor.role}"{index < movie.actors.length - 1 && ', '}
										</p>
									))
								) : (
									<p>N/A</p>
					...
					similarMovies.map((similarMovie) => {
						...
						return (
							<Col
								key={similarMovie.id}
								md={4}
								className='mb-5 g-3 d-flex justify-content-center'>
								<MovieCard
									movie={similarMovie}
